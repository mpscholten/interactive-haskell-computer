...
-- | Run scanFixityDecls starting from the given table, asserting that
-- the scan succeeds (no ParseError thrown).
scanFixOk :: FixityTable -> ByteString -> Expectation
scanFixOk tbl bs = do
...
spec = describe "Hs2010 — Fixity & type signatures" $ do
    ...
    describe "3.8 fixity declarations" $ do
        ...
        it "3.8.3 `infix 4 ==` parses without error" $
            scanFixOk defaultFixityTable "infix 4 ==\n"
