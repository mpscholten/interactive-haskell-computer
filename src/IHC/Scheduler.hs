...
resetPerRunGlobals = do
    ...
    when memDebugEnabled $ do
        ...
        when (n `mod` memDebugEvery == 0) $ do
            ...
            -- #179's base had a process-global 'globalEarlyBuiltinsRef'
            -- (an 'IORef (Set ByteString)') here as the third suspected
            -- cross-fixture retainer.  Master replaced that mechanism
            -- (commit d15bc69, "bound the per-FV discovery chase so Eq
            -- can be source-loaded") with the static CAF
            -- 'perFVChaseShortCircuit' — the early-builtins short-circuit
            -- set is now a constant, not an accumulating mutable ref, so
            -- it is by construction not a leak vector.  Report its size
            -- so the @[ihc:mem]@ dump keeps its 3-metric shape; it reads
            -- as a flat constant (which is the correct, post-d15bc69
            -- signal — a *rising* value would be the regression).
            let eb = Set.size perFVChaseShortCircuit
