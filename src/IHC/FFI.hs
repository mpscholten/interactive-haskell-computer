...
-- The two top-level 'IORef' CAFs below ('openLibs', 'symbolCache')
-- intentionally stay as process-wide state rather than moving onto
-- 'IHC.Runtime.IHCRuntime'.  They mirror @dlopen@ and @dlsym@ results,
-- which are themselves process-scoped at the OS level: multiple
-- 'IHCRuntime' instances running in the same process would see the
-- same loaded libraries and resolve the same symbols regardless of
-- which runtime initiated the load.  Threading a per-runtime ref
-- through every FFI call site would add noise without changing
-- behaviour.  Every other CAF the interpreter used to carry has
-- already migrated to 'IHCRuntime'.
