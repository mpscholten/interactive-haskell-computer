...
builtins reg =
    ...
    -- try / handle: graduated to source-load (#171); bracket / bracket_ /
    -- bracketOnError / finally / onException: graduated to source-load
    -- (#166).  All interpret from GHC.Internal.Control.Exception.Base /
    -- GHC.Internal.IO source; no host shim by these names.
    , ("throwTo",         throwToB)
...
-- bracket / bracket_ / bracketOnError / finally / onException: no host
-- shim.  Source-loaded from GHC.Internal.Control.Exception.Base, which
-- expresses them via mask / catch / throwIO (all of which bottom out on
-- registered primops or the catchB host catch).  See #166.
