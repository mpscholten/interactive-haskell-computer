  ...
  where
    ...
    parseClassHead cur0 = scanHead cur0 Nothing [] [] (0 :: Int) Nothing False
      where
        ...
        --   When @=>@ fires, these become the superclass list and
        ...
        -- whether we ever saw `=>`.
        --
        -- * Saw `=>`: @mCls@ is the class name (captured after the
...
tyConToFFI c _ = case c of
    ...
    "Int#"     -> Just FFIInt64
    "Word#"    -> Just FFIWord64
    "ThreadId#" -> Just FFIThreadId
    "CLLong"   -> Just FFIInt64
    "CULLong"  -> Just FFIWord64
...
-- Cursor/layout collection remains scanner-local; only the pure grammar is
-- shared with elaboration.
