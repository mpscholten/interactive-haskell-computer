...
- **`plusForeignPtr`** still shimmed despite being a Haskell definition (`plusForeignPtr (ForeignPtr addr c) (I# d) = ForeignPtr (plusAddr# addr d) c`). The historical "removing it regresses ~7 fixtures" claim was a *symptom*, not a cause — the parser bug in `parseSubNames` (operator-group sub-imports like `NonEmpty ((:|))` corrupting the import-parse cursor) silently dropped every `import GHC.ForeignPtr (plusForeignPtr)` in `Data.ByteString.Internal.Type`, leaving the source-loaded body unreachable. The parser bug is now fixed. The remaining blocker is *downstream*: callers reach the source body with `Int`-typed args routed through `fromInteger`, arriving as `VCon "IS" [VInt n]` rather than bare `VInt n` — the `Integer` arithmetic chain itself produces wrong values (e.g. `BSC.replicate` zeroing its output). Until the IS-arithmetic path is fixed, removing the shim regresses a small set of fixtures. Comment in `Builtins.hs` documents the gap.
